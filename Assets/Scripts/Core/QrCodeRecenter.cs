...
public class QrCodeRecenter : MonoBehaviour {
    ...
    private void Update()
    {
        ...
        {
            TargetFacade currentTarget = targetHandler.GetCurrentTargetByTargetText("Outside3Qr");
        ...
        {
            TargetFacade currentTarget = targetHandler.GetCurrentTargetByTargetText("Room2Qr");
            if (currentTarget != null)
            {
                // Reset position and rotation of ARSession
                session.Reset();
    ...
    private void SetQrCodeRecenterTarget(string targetText) {
        ...
        if (currentTarget != null) {
            ...
            sessionOrigin.transform.position = currentTarget.transform.position;
            sessionOrigin.transform.rotation = currentTarget.transform.rotation;
            destDropdown.gameObject.SetActive(scanningEnabled);
            ExpandDropdown();
        }
    }
    private void ExpandDropdown()
    {
        // Trigger a click event to expand the dropdown
        EventSystem.current.SetSelectedGameObject(destDropdown.gameObject);
        destDropdown.OnPointerClick(new PointerEventData(EventSystem.current));
    }
    ...
    public void ToggleScanning() {
        ...
        //if(scanningEnabled )
        //{
        //    qrTextField.text = "QR Scan enabled";
        //} else
        //{
        //    qrTextField.text = "QR Scan disabled";
        //}
    }
}
